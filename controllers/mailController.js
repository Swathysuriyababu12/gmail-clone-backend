 ...
 const getEmails = async (request, response) => {
  try {
    ...
    return response
      .status(200)
      .json({
        message: `${request.params.type} Emails Fetched successfully`,
        success: true,
        emails,
      });
  } catch (error) {
    return response
      .status(500)
      .json({ message: "Error while Getting Emails.", error: error.message });
  }
};

 const saveDraftEmails = async (request, response) => {
  try {
    if (request.body) {
      await Mail.create(request.body);
      ...
      return response
        .status(201)
        .json({ message: "Draft Email saved successfully", success: true });
    }
  } catch (error) {
    return response
      .status(500)
  ...
  try {
    if (request.body) {
      ...
      return response
        .status(200)
        .json({
          message: "Draft Emails moved to bin successfully",
          success: true,
        });
    }
  } catch (error) {
    return response
      .status(500)
      .json({
        message: "Error while moving emails to bin.",
        error: error.message,
      });
  }
};
 const toggleStarredEmails = async (request, response) => {
  try {
    if (request.body) {
      ...
      return response
        .status(200)
        .json({ message: "email starred toggle successfully", success: true });
    }
  } catch (error) {
    return response
      .status(500)
      .json({
        message: "Error while toggle strarred emails Email.",
        error: error.message,
      });
  }
};
 ...
 const deleteEmails = async (request, response) => {
  try {
    if (request.body) {
      ...
      return response
        .status(200)
        .json({ message: "Emails deleted successfully", success: true });
    }
  } catch (error) {
    return response
      .status(500)
      .json({ message: "Error while deleting emails.", error: error.message });
  }
};

module.exports = { saveSentEmails,getEmails,saveDraftEmails,moveEmailsToBin,deleteEmails,toggleStarredEmails };
