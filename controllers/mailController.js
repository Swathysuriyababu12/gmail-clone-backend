const Email = require("../models/mailSchema.js");
...
const sendEmail = async (request, response) => {
  try {
    ...
    const foundAccountOfReceiver = await Account.findOne({
      email: request.body.to,
    });
    if (foundAccountOfReceiver) {
      foundAccountOfReceiver.mailbox.inbox.push(savedEmailOut._id);
      response.status(201).json({
        message: "Email sent",
        sent: savedEmailOut,
      });
      await foundAccount.save();
      await foundAccountOfReceiver.save();
    } else {
      response.status(500).send("Invalid Receiver Email");
    }
...
const saveDraftEmails=async(request, response, next)=> {
  try {
    // construct new draft
...
const  moveEmailsToBin=async(request, response, next)=> {
  try {
    // find user by ID
